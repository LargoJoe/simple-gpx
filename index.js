...
app.post("/", upload.single("fileinput"), function (req, res) {
  ...
  var gpx_filename = req.file.originalname.substr(
    0,
  ...
  parseString(gpx, function (err, result) {
    if (err) {
      res.send(
        "That doesn't appear to be a GPX file. Use the back arrow and select a valid GPX."
      );
      return;
    }
    ...
    if (!result.gpx.hasOwnProperty("trk")) {
      res.send(
        "The GPX file doesn't appear to contain a track. Use the back arrow and select another GPX."
      );
      return;
    }
    ...
    for (var i = 0; i < tl; ++i) {
      ...
      if (req.body.distancealarm !== "None") {
        var distancealarm = parseInt(req.body.distancealarm) * 1000;
        ...
        for (var l = 0; l < accum_lengths.length; l++) {
          if (accum_lengths[l] > alarm_distance) {
            ...
            var proximity = {
              "wptx1:WaypointExtension": {
                "wptx1:Proximity": req.body.distanceproximityalarm,
              },
            };
            wpt.extensions = [];
            wpt.extensions.push(proximity);

           
            if (typeof result.gpx.wpt !== "undefined") {
              result.gpx.wpt.push(wpt);
            } else {
              result.gpx.wpt = [];
              result.gpx.wpt.push(wpt);
            }
      ...
      if (!isNaN(parseInt(input_accuracy))) {
         ...
         */
        if (
          req.body.splittrk === "yes - by waypoint" &&
          result.gpx.wpt !== "undefined"
        ) {
        ...
        for (var s = 0; s < split_pts.length; ++s) {
          ...
          if (input_accuracy == 500 || input_accuracy == 10000) {
            ...
            while (loop === true) {
              ...
              if (
                simple_pts.length > input_accuracy ||
                simple_pts.length < input_accuracy * 0.99
              ) {
    ...
    if (input_accuracy === "route") {
      ...
      if (typeof result.gpx.rte === "undefined") {
        ...
        if (typeof total_length === "undefined") {
          var accumulated_lengths = accumulatedLengths(pts);
          var total_length_km =
            accumulated_lengths[accumulated_lengths.length - 1] / 1000;
        } else {
        ...
        while (loop === true) {
          ...
          if (
            simple_rtes.length > input_accuracy_rte * 1.2 ||
            simple_rtes.length < input_accuracy_rte * 0.8
          ) {
        ...
        var routes_generated =
          "Routes generated: " +
          r +
          ", average points per route: " +
          simple_rtes.length / r;
      }
    ...
    if (!isNaN(parseInt(input_accuracy))) {
      txt +=
        ...
        split_pts.length +
        "\r\n" +
        "Original trackpoints: " +
        pts.length +
        ", simplified trackpoints: " +
        total_points +
        ", average trackpoints per track: " +
        parseInt(total_points / split_pts.length) +
        "\r\n";
    }
